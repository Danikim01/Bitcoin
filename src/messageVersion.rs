...
impl Message for Version {
    ...
    fn from_bytes(&self, bytes: &[u8]) -> Result<Box<dyn Message>, String> { // since this is a method, change it to modify self instead of returning a new Message
        let mut cursor = Cursor::new(bytes);
        let mut version = [0 as u8; 4];
        let mut services = [0 as u8; 8];
        let mut timestamp = [0 as u8; 8];
        let mut addr_recv_services = [0 as u8; 8];
        let mut addr_recv_ip = [0 as u8; 16];
        let mut addr_recv_port = [0 as u8; 2];
        let mut addr_trans_port = [0 as u8; 2];
        let mut nonce = [0 as u8; 8];
        let user_agent_size: u64;
        let mut start_height = [0 as u8; 4];
        let mut relay = [0 as u8; 1];

        cursor.read_exact(&mut version).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut services).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut timestamp).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut addr_recv_services).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut addr_recv_ip).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut addr_recv_port).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut addr_trans_port).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut nonce).map_err(|error| {error.to_string()})?;

        let mut byte = [0 as u8; 1];
        cursor.read_exact(&mut byte).map_err(|error| {error.to_string()})?;
        if byte[0] < 0xFD {
            ...
            let mut user_agent_bytes = vec![0 as u8; buffer_size];
            cursor.read_exact(&mut user_agent_bytes);
            user_agent_size = u64::from_be_bytes(vec_to_arr(user_agent_bytes));
        }
        let mut user_agent = vec![0 as u8; user_agent_size as usize];
        cursor.read_exact(&mut user_agent).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut start_height).map_err(|error| {error.to_string()})?;
        cursor.read_exact(&mut relay).map_err(|error| {error.to_string()})?; // pending: this field should be optional

        Ok(Box::new(Version::new(
            ...
            std::str::from_utf8(&user_agent).map_err(|error| {error.to_string()})?.to_string(),
