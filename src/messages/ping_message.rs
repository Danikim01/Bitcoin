...
use crate::messages::{Message, MessageHeader, Serialize, utility::StreamRead, constants::{commands::PONG, config::MAGIC}};
...
impl Ping {
    ...
    pub fn pong(bytes: &[u8]) -> io::Result<Vec<u8>> {
        ...
        let message_header = MessageHeader::new(MAGIC, PONG.to_string(), bytes.len() as u32, checksum);
